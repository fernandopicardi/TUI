...
const MCPPanel: React.FC<Props> = ({ worktreePath }) => {
  ...
  const loadServers = useCallback(() => {
    if (!window.agentflow?.mcp) return
    window.agentflow.mcp.getConfig(worktreePath)
      .then(setServers)
      .catch(() => {})
  }, [worktreePath])
  ...
  const handleRemove = async (name: string, scope: 'global' | 'project') => {
    ...
    const result = await window.agentflow.mcp.removeServer(worktreePath, name, scope)
  ...
  const handleAdd = async () => {
    ...
    const result = await window.agentflow.mcp.addServer(worktreePath, server, newScope)
    if (result.success) {
